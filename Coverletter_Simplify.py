...
def main():
    """Main function that waits for key presses to trigger cover letter generation or exit."""
    print("Program is running... Press '`' to generate a cover letter, 'F9' to exit.")

    while True:
        event = keyboard.read_event()

        if event.event_type == keyboard.KEY_DOWN:
            if event.name == "`":  # ` (backtick) key to generate cover letter
                ...
                if not cover_letter_from_gpt:
                    ...
                    continue  # Go back to waiting for the next key press
                
                company_name = get_company_name(cover_letter_from_gpt)
                insert_text_into_word(word_file, company_name)

                print("\nCover letter saved. Press '`' again to generate a new one.\n")

            elif event.name == "f9":  # F9 key to exit
                print("\nF9 detected! Exiting program...\n")
